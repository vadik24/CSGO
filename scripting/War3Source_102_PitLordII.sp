...
public OnWar3LoadRaceOrItemOrdered(num)
{
	if(num==228)
	{
		thisRaceID = War3_CreateNewRace( "Azgalor - Pit Lord II", "azgapitlord" );
		SKILL_FIRE = War3_AddRaceSkill( thisRaceID, "Fire Storm", "Calls down up to 3 damages(per cast) of fire that damage enemy units in an area\n Each damage deals damage and then burns enemies for 2 seconds.", false, 4 );	
...
public OnMapStart() {
  ...
  PrecacheModel("models/player/slow/general_raam/slow.mdl", true);
  PrecacheModel("models/player/slow/theron_guard/slow.mdl", true);
}
...
public OnWar3EventSpawn(client)
{
	...
	{ 
		...
		if(ValidPlayer(client,true))
		{
			...
			{
			SetEntityModel(client, "models/player/slow/general_raam/slow.mdl");
			}
			if(GetClientTeam(client)==2)
			{
			SetEntityModel(client, "models/player/slow/theron_guard/slow.mdl");
			}
		}
	}
}
	...
	if(ValidPlayer(victim,true)&&ValidPlayer(attacker,true)){
		...
		//{
			if(War3_GetRace(attacker)==thisRaceID && War3_GetRace(victim)!=War3_GetRaceIDByShortname("azgalor"))
			{
				...
				if(showmsg)
				{
					PrintHintText(attacker,"Fire Storm:\nCasted %i waves of Fire"teammount);
				}
			}
		//}
	}
		...
		if(skill_level>0)
		{
			if(!Silenced(client))
			{
				if(War3_SkillNotInCooldown(client,thisRaceID,SKILL_PIT,true))
				{
					...
					EmitSoundToAll(riftsnd,client);
					War3_CooldownMGR(client,2.5,thisRaceID,SKILL_PIT, _,_);
